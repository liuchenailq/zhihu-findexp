...
def make_lgb_data(mode, isSave =True,hasFile = False):
    ...
    if hasFile == True:
        return pd.read_csv('../datasets2/feature/' + mode + '/nn_data.csv')

    if mode == 'train':
        mode1 = mode + '_use'
        data = pd.read_csv('../datasets3/invite_' + mode1 + '.csv')
    elif mode == 'test_B':
        data = pd.read_csv('../datasets3/test_final.csv')
    else:
        mode1 = mode
        data = pd.read_csv('../datasets1/invite_' + mode1 + '.csv')
    ...
    data = data.drop(['merge_qid_count', 'merge_uid_count','merge_qid_min_day', 'merge_uid_min_day', 'merge_qid_day_nuinque', 'merge_uid_day_nuinque'], axis=1)
...
def make_number_feature(mode,data):
    ...
    load_number_feature('merge_uid_rank', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_qid_rank', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_uid_day_rank', DigtEncoder('linear'), mode, data, feat_list)

    load_number_feature('merge_uid_past_invite_time', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_qid_past_invite_time', DigtEncoder('linear'), mode, data, feat_list)


    load_number_feature('merge_uid_time_past', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_uid_time_feature', DigtEncoder('linear'), mode, data, feat_list)

    load_number_feature('merge_qid_diff_first_invite_time', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_uid_diff_first_invite_time', DigtEncoder('linear'), mode, data, feat_list)

    load_number_feature('merge_uid_avg_stack_count', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_qid_avg_stack_count', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_qid_cur_diff_first', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_uid_cur_diff_first', DigtEncoder('linear'), mode, data, feat_list)
    ...
    data['merge_uid_time_past'] = data['merge_uid_time_past'].apply(lambda x: x if x < 30 else np.NAN)
    data['merge_uid_time_feature'] = data['merge_uid_time_feature'].apply(lambda x: x if x <= 4 else np.NAN)

    data['merge_uid_past_invite_time'] = data['merge_uid_past_invite_time'].apply(lambda x: x if x < 30 else np.NAN)
    data['merge_qid_past_invite_time'] = data['merge_qid_past_invite_time'].apply(lambda x: x if x < 30 else np.NAN)



    load_number_feature('time_diff', DigtEncoder('linear'), mode, data, feat_list)

    load_number_feature('merge_qid_count', DigtEncoder('log1p'), mode, data, feat_list)
    load_number_feature('merge_uid_count', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_uid_day_count', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_uid_day_hour_count', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_uid_min_day_count', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_qid_day_count', DigtEncoder('log1p'), mode, data, feat_list)
    load_number_feature('merge_qid_day_hour_count', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_qid_min_day_count', DigtEncoder('linear'), mode, data, feat_list)

    load_number_feature('merge_qid_day_shour_count', DigtEncoder(), mode, data, feat_list)
    load_number_feature('merge_qid_day-1_shour_count', DigtEncoder(), mode, data, feat_list)
    load_number_feature('merge_qid_day-1_count', DigtEncoder(), mode, data, feat_list)
    load_number_feature('merge_qid_day+1_count', DigtEncoder(), mode, data, feat_list)

    data['qid_next_today_count_dif'] = data['merge_qid_day+1_count'] - data['merge_qid_day_count']
    data['qid_last_today_count_dif'] = data['merge_qid_day_count'] - data['merge_qid_day-1_count']


    load_number_feature('merge_uid_day_nuinque', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_qid_day_nuinque', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_uid_min_day', DigtEncoder('linear'), mode, data, feat_list)
    load_number_feature('merge_qid_min_day', DigtEncoder('linear'), mode, data, feat_list)
    data['qid_count_nunique'] = np.divide(data['merge_qid_count'],data['merge_qid_day_nuinque'])
    data['uid_count_nunique'] = np.divide(data['merge_uid_count'], data['merge_uid_day_nuinque'])
    data['qid_by_day'] = np.divide(data['merge_qid_rank'] - 1, data['time_diff'] + 0.00001)

    load_number_feature('merge_topic_day_hour_uid_nunique_max', DigtEncoder(), mode, data, feat_list)
    load_number_feature('merge_topic_uid_nunique_max', DigtEncoder(), mode, data, feat_list)
    load_number_feature('merge_topic_qid_nunique_max', DigtEncoder(), mode, data, feat_list)
    load_number_feature('merge_topic_label_count_max', DigtEncoder(), mode, data, feat_list)

    load_number_feature('merge_topic_day_hour_uid_nunique_sum', DigtEncoder(), mode, data, feat_list)


    data['cur_day_diff'] = data['merge_qid_day_count'] - data['merge_qid_min_day_count']
    ...
    load_number_feature('merge_recent_7_day_click_rate', DigtEncoder(), mode, data, feat_list,fillnas=0)
    load_number_feature('merge_ans_dif_1', DigtEncoder(), mode, data, feat_list)
    load_number_feature('merge_ans_dif_7', DigtEncoder(), mode, data, feat_list)
    load_number_feature('merge_ans_dif_topic_1', DigtEncoder(), mode, data, feat_list)
...
def load_muti_feature(feat ,mode, data,  maxlen = 5 ,use_init = '-', feat_list = None, muti_p = None):
    ...
    t = pd.read_csv('../datasets3/feature/' + mode + '/' + feat + '.csv',skip_blank_lines=False)
    t = t.fillna('0')
    data[feat] = t
