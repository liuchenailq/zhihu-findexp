...
class_feat = ['m_sex', 'm_access_frequencies', 'm_twoA', 'm_twoB', 'm_twoC', 'm_twoD', 'm_twoE', 'm_categoryA',
              ...
              'uid_seq_feature', 'uid_seq_feature_3', 'new_memberID_day_count',
              ...
              'new_uid_after_invite_diff', 'new_qid_after_invite_diff', 'new_qid_last_invite_diff','new_uid_duration_days', 'new_uid_to_first_days'
            ]

fixlen_number_columns = ['m_salt_score', 'm_num_atten_topic', 'q_num_title_chars_words', 'q_num_desc_chars_words', 'q_num_desc_words',
                         'q_num_title_words', 'days_to_invite', 'memberID_times', 'memberID_neg_times', 'memberID_pos_times', 'member_answer_times',
                         ...
                         'member_continuous_reject_times', 'member_continuous_accept_times', 'uid_ans_topic_count', 'new_uid_rank'
                        ]
...
y_train = train[train['day'] <= 3864]['label'].values
X_train = train[train['day'] <= 3864][all_features].values
...
model_lgb = LGBMClassifier(boosting_type='gbdt', num_leaves=120, learning_rate=0.01, n_estimators=3000,
...
test['result'] = y_pred
test[['questionID', 'memberID', 'time', 'index', 'result']].to_csv('/cu04_nfs/lchen/data/data_set_0926/features/pred_test.txt', index=False, sep='\t')
print("结果保存成功！")
