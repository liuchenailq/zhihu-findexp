...
t1 = total_extend.groupby(['topic'])['memberID'].agg(['count']).reset_index().rename(columns={'count': 'topic_count'})
...
for stat in fea_list:
    ...
    # 统计话题和用户属性的交叉量
    t = total_extend.groupby(['topic', stat])['questionID'].agg(['count']).reset_index().rename(
        columns={'count': 'sum_count'})
    t = pd.merge(t, t1, how='left', on='topic')  # topic stat sum_count topic_count
    ...
    t = t.drop(['topic_count', 'sum_count'], axis=1)  # topic stat ratio
...
target_data.to_csv(path + "features/topic_user_ratio_" + job + ".txt", sep='\t', index=False)
print("finish!")
