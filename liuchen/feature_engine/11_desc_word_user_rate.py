...
target_data = target_data.merge(question_data[['questionID', 'q_desc_words']], how='left', on='questionID')
feature_data = feature_data.merge(question_data[['questionID', 'q_desc_words']], how='left', on='questionID')
...
# 将q_desc_words拆开 一条记录变为多条
total_extend = feature_data['q_desc_words'].str.split(' ', expand=True).stack() \
        .reset_index(level=0).set_index('level_0') \
        .rename(columns={0: 'desc_word'}).join(feature_data.drop('q_desc_words', axis=1)) \
        .reset_index(drop=True)

topic_df = target_data['q_desc_words'].str.split(' ', expand=True)
...
for fea in fea_list:
    feature_name = "desc_word_" + fea
    print(feature_name)
    total_extend[feature_name] = total_extend["desc_word"].map(str) + total_extend['join'].map(str) + total_extend[fea].map(str)
...
target_data.to_csv(path + "features/desc_word_user_rate_" + job + ".txt", sep='\t', index=False)
print("finish!")
