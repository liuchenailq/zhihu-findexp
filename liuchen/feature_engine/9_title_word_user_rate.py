...
feature_data = feature_data.merge(question_data[['questionID', 'q_title_words']], how='left', on='questionID')
...
total_extend = feature_data['q_title_words'].str.split(' ', expand=True).stack() \
        .reset_index(level=0).set_index('level_0') \
        .rename(columns={0: 'title_word'}).join(feature_data.drop('q_title_words', axis=1)) \
...
for fea in fea_list:
    feature_name = "title_word_" + fea
    print(feature_name)
    total_extend[feature_name] = total_extend["title_word"].map(str) + total_extend['join'].map(str) + total_extend[fea].map(str)
...
target_data.to_csv(path + "features/title_word_user_rate_" + job + ".txt", sep='\t', index=False)
print("finish!")
